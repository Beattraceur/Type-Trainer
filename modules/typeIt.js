...
export function typeQuest(questArr,activeKeyArr,keyEvent){
  if(keyEvent === 'pioneer'){ ///first call of typeQuest 
  ...
  ///############# User Mission Start
  if(!questStorage.runningQuest && keyEvent === ' ' && !questStorage.questEnded){
    el('#key57').innerText = '';
    //console.log('start');
  ...
  ///############# ongoing typeQuest 
  if(questStorage.runningQuest){
    timeLeft = questStorage.endTime - Date.now();
    updateTimer(timeLeft);
    if(timeLeft <= 0){ /// checks if time is over
      const zero = 0;
      updateTimer(zero);
      ...
      clearInterval(globals.cursorID);
      displayResult();
    }else //false when time over
    if(keyEvent === activeChar){
  ...
  if(questStorage.questEnded && delayDone){ ///go to menu when typing but only if delay has passed
    globals.layer = 2;
    inputRouter('pioneer');
  }else{
...
let sum = 0;
  ...
  resultObj.accuracy = Math.round((questStorage.pastArr.length/(questStorage.pastArr.length + questStorage.errorArr.length))*100);

  resultObj.averageSpeed = Math.round((sum / questStorage.pastArr.length)*100)/100;
  //console.log(sum);
      ...
      case 2: ///############# Test 2 Options
        ...
        if(selTest.currentSel === 0){
         // console.log('no sel');
          ...
          ///############# Stay with Option1
          if(keyEvent === selTest.activeArr[0]){
            selTest.opt1ArrPassive.push(selTest.activeArr.shift()); //remove first value
            el('#opt1').innerText = selTest.activeArr.join('');
